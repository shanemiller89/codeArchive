...
export default class ToolArchiveForm extends Component {
  state = {
    title: "",
    link: "",
    libraryId: null,
  ...
  googleSubmit = () => {
    ...
    this.props.addArchive(archive).then(newArchive => {
      this.props.addToolArchive({
        libraryId: this.props.toolId,
        archiveId: newArchive.id
      });
      const searchTerm = `${this.props.toolTitle} ${this.state.title}`
